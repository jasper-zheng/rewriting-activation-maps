...
class SynthesisBlock(tf.keras.layers.Layer):
    def __init__(self, in_ch, fmaps, res, **kwargs):
        ...
        self.conv_0 = ModulatedConv2D(in_res=res//2, in_fmaps=self.in_ch, fmaps=self.fmaps, kernel=3, up=True, down=False,
                                      demodulate=True, resample_kernel=[1, 3, 3, 1], gain=self.gain, lrmul=self.lrmul,
                                      fused_modconv=True, name=f'conv_0_{res}')
        self.apply_noise_0 = Noise(name=f'noise_0_{res}')
        self.apply_bias_act_0 = BiasAct(lrmul=self.lrmul, act='lrelu', name=f'bias_act_0_{res}')

        # conv block
        self.conv_1 = ModulatedConv2D(in_res=res, in_fmaps=self.fmaps, fmaps=self.fmaps, kernel=3, up=False, down=False,
                                      demodulate=True, resample_kernel=[1, 3, 3, 1], gain=self.gain, lrmul=self.lrmul,
                                      fused_modconv=True, name=f'conv_1_{res}')
        self.apply_noise_1 = Noise(name=f'noise_1_{res}')
        self.apply_bias_act_1 = BiasAct(lrmul=self.lrmul, act='lrelu', name=f'bias_act_1_{res}')
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        # print(f'conv0, res: {self.res}, shape: {x.shape}')
        ...
        # print(f'conv1, res: {self.res}, shape: {x.shape}')
...
class Synthesis(tf.keras.layers.Layer):
    ...
    def call(self, inputs, training=None, mask=None, return_intermediate_y = False):
        ...
        if return_intermediate_y:
          return images_out, self.intermediate_y
        else:
          return images_out

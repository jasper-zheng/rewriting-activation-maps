...
class Generator(tf.keras.Model):
    ...
    def call(self, inputs, ret_w_broadcasted=False, truncation_psi=1.0, truncation_cutoff=None, training=None, mask=None, return_intermediate_y=False):
        ...
        #print(w_broadcasted.shape)

        if return_intermediate_y:
          image_out, intermediate_y = self.synthesis(w_broadcasted,return_intermediate_y=return_intermediate_y)
        else:
          image_out = self.synthesis(w_broadcasted)
        
        if return_intermediate_y and ret_w_broadcasted:
          return image_out, w_broadcasted,intermediate_y
        elif ret_w_broadcasted:
          return image_out, w_broadcasted
        elif return_intermediate_y:
          return image_out, intermediate_y
        else:
          return image_out
...
def load_and_restore(ckpt_dir, z_dim = 512, w_dim = 512, labels_dim = 0, n_mapping = 8, resolution = 1024):
  g_params = {
            'z_dim': z_dim,
            'w_dim': w_dim,
            'labels_dim': labels_dim,
            'n_mapping': n_mapping,
            'resolutions': resolutions[:resolutions.index(resolution)+1],
            'featuremaps': featuremaps[:resolutions.index(resolution)+1],
        }
